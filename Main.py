...
class Main(QMainWindow):
	def __init__(self):
		...
		self.setGeometry(480, 270, 960, 540) # 크기, 위치 설정  #debug#
		self.show()
	...
	def loadProject(self, project_name:str, request_name:str=None):
		...
		try:
			...
			Global.res[project_name][request_name]=Response()  # 응답 탭
			Global.res[project_name][request_name].response_tab_content.setParent(self.response_tab)  # 응답 탭 UI 갈아끼우기
	...
	def createProject(self):
		...
		if dialog.return_data:
			# 이름 유효 검사
			if File.isInvalidName(dialog.return_data[1]):  # 유효하지 않은 이름
				#todo# 알림
				return
...
class CreateProjectDialog(QDialog):
	...
	def initUI(self):
		...
		# -> 기본 파일 경로 설정
		#todo# -> 매개변수로 Global.default_project_path 넣기
		# 텍스트 변경
	...
	def checkInvaildName(self):
		if File.isInvalidName(self.pjname_if.text(), self.pjpath_if.text(), 'p'):
			...
			self.crt_btn.setDisabled(True)  # 버튼 비활성화
		else:
